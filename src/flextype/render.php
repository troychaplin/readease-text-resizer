<?php
/**
 * Server-side rendering for the FlexType Text Resizer block.
 *
...
<div <?php echo wp_kses_post( $wrapper_attributes ); ?>>
	...
	<?php if ( 'buttons' === $control_style ) : ?>
		<div class="wp-block-flextype-text-resizer__controls wp-block-flextype-text-resizer__controls--buttons" role="group" aria-label="<?php esc_attr_e( 'Text size controls', 'flextype' ); ?>">
			<?php foreach ( $scales as $index => $scale ) : ?>
				...
				$button_class = 'wp-block-flextype-text-resizer__button';
				if ( $is_active ) {
					$button_class .= ' wp-block-flextype-text-resizer__button--active';
				}

				// Determine button label.
				if ( 0 === $index ) {
					$aria_label = __( 'Decrease text size', 'flextype' );
				} elseif ( count( $scales ) - 1 === $index ) {
					$aria_label = __( 'Increase text size', 'flextype' );
				} else {
					$aria_label = __( 'Reset text size', 'flextype' );
				}
				?>
				<button
					type="button"
				...
				>
					<span class="wp-block-flextype-text-resizer__button-text">A</span>
				</button>
			<?php endforeach; ?>
		</div>
	...
	<?php elseif ( 'slider' === $control_style ) : ?>
		...
		<div class="wp-block-flextype-text-resizer__controls wp-block-flextype-text-resizer__controls--slider">
			...
			/>
			<span class="wp-block-flextype-text-resizer__slider-label wp-block-flextype-text-resizer__slider-label--max">A</span>
		</div>

	<?php elseif ( 'icons' === $control_style ) : ?>
		<div class="wp-block-flextype-text-resizer__controls wp-block-flextype-text-resizer__controls--icons" role="group" aria-label="<?php esc_attr_e( 'Text size controls', 'flextype' ); ?>">
			<button
				type="button"
				class="wp-block-flextype-text-resizer__icon-button"
				data-action="decrease"
				aria-label="<?php esc_attr_e( 'Decrease text size', 'flextype' ); ?>"
			>
			...
			<button
				type="button"
				class="wp-block-flextype-text-resizer__icon-button wp-block-flextype-text-resizer__icon-button--active"
				data-action="reset"
				aria-label="<?php esc_attr_e( 'Reset text size', 'flextype' ); ?>"
				aria-pressed="true"
			>
				<span class="wp-block-flextype-text-resizer__icon-text">A</span>
			</button>
			<button
				type="button"
				class="wp-block-flextype-text-resizer__icon-button"
				data-action="increase"
				aria-label="<?php esc_attr_e( 'Increase text size', 'flextype' ); ?>"
			>
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" aria-hidden="true" focusable="false">
