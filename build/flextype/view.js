...
(function () {
  ...
  const TEXT_ELEMENTS = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'li', 'td', 'th', 'dd', 'dt', 'blockquote', 'figcaption'];

  // Template part selectors to exclude when scaleScope is 'exclude-template'
  const TEMPLATE_PART_SELECTORS = ['header', '.site-header', '.wp-block-template-part[data-slug="header"]', 'footer', '.site-footer', '.wp-block-template-part[data-slug="footer"]', 'nav', '.site-navigation', '.wp-block-navigation', '#wpadminbar'];
  ...
  function buildExclusionSelector(element) {
    const exclusions = TEMPLATE_PART_SELECTORS.map(tpl => `:not(${tpl} ${element})`).join('');
  ...
  function applyScale(scale, scaleScope, customTarget) {
    ...
    if (customTarget && customTarget.trim() !== '') {
      ...
      selector = TEXT_ELEMENTS.map(el => buildExclusionSelector(el)).join(', ');
    } else {
      // Full page: target all text elements but exclude admin bar
      selector = TEXT_ELEMENTS.map(el => `body ${el}:not(#wpadminbar ${el})`).join(', ');
    }
  ...
  function updateButtonStates(container, scale) {
    const buttons = container.querySelectorAll('.wp-block-flextype-text-resizer__button');
    buttons.forEach(button => {
      const buttonScale = parseFloat(button.dataset.scale);
      const isActive = Math.abs(buttonScale - scale) < 0.01;
      button.classList.toggle('wp-block-flextype-text-resizer__button--active', isActive);
  ...
  function updateIconStates(container, scale, minScale, maxScale) {
    ...
    if (resetButton) {
      ...
      resetButton.classList.toggle('wp-block-flextype-text-resizer__icon-button--active', isDefault);
      resetButton.setAttribute('aria-pressed', isDefault ? 'true' : 'false');
    }
  ...
  function updateSliderValue(container, scale) {
    const slider = container.querySelector('.wp-block-flextype-text-resizer__slider');
    if (slider) {
      slider.value = scale;
    }
  }
  ...
  function updateDropdownValue(container, scale) {
    const select = container.querySelector('.wp-block-flextype-text-resizer__select');
    if (select) {
      select.value = scale;
    }
  }
   ...
   * @param {number}      maxScale     Maximum scale
   */
  function initIcons(container, scaleScope, customTarget, currentScale, minScale, maxScale) {
    const step = 0.1;
    const decreaseBtn = container.querySelector('[data-action="decrease"]');
  ...
  function initBlock(container) {
    ...
    if (controlStyle === 'dropdown') {
      ...
      initIcons(container, scaleScope, customTarget, currentScale, minScale, maxScale);
    }
  }
  ...
  function handleStorageChange(event) {
    ...
    const scaleScope = firstBlock ? firstBlock.dataset.scaleScope || 'exclude-template' : 'exclude-template';
    const customTarget = firstBlock ? firstBlock.dataset.targetSelector || '' : '';
    applyScale(scale, scaleScope, customTarget);
    updateAllControls(scale);
  }
